<?php
    ...
    if(2>abs(strtotime($StopTime.':00') - strtotime($StartTime.':00'))/60/60/24 and 2<abs(strtotime($StopTime.':00') - strtotime($StartTime.':00'))/60/60){
        ...
        for($i=0;$i<count($result);$i+=2){
            ...
            $dat[$i/2][1]=floatval($result[$i]['MeasurandValue']);
            if($result[$i]['MeasurandValue']>=$result[$i+1]['MeasurandValue']){
                $dat[$i/2][2]=floatval($result[$i]['MeasurandValue']);
                $dat[$i/2][3]=floatval($result[$i+1]['MeasurandValue']);
            }
            if($result[$i]['MeasurandValue']<$result[$i+1]['MeasurandValue']){
                $dat[$i/2][2]=floatval($result[$i+1]['MeasurandValue']);
                $dat[$i/2][3]=floatval($result[$i]['MeasurandValue']);
            }
            $dat[$i/2][4]=floatval($result[$i+1]['MeasurandValue']);
    ...
    if(2<abs(strtotime($StopTime.':00') - strtotime($StartTime.':00'))/60/60/24 and 7>abs(strtotime($StopTime.':00') - strtotime($StartTime.':00'))/60/60/24){
        $m=M('table_hour');
        $data=$m->where('AId='.$AId)->select();
        $h=count($data);
        for($i=0;$i<$h;$i++){
